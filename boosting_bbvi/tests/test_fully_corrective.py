"""Smoke test for fully corrective step size selection of Frank Wolfe."""
# TODO set seed
import sys
import os
...
def main():
    ...
    qxs = [
        MultivariateNormalDiag(
            loc=[scipy.stats.norm.rvs(1)],
            scale_diag=[scipy.stats.norm.rvs(1)]) for i in range(10)
    ]

    truth = [
        MultivariateNormalDiag(
            loc=mixture_model_relbo.mus[i],
    ...
    mix = Mixture(
        cat=Categorical(probs=[1. / len(qxs)] * len(qxs)), components=qxs)
